#The IPFMA partnership database has a different page for each project; each project page 
#has a description and a side bar with different category content.
#The project pages are not always exactly the same, mostly in terms of the side bar categories.
#Scanning through I've identified what categories I'm interested in, and we will extract those 
#directly, along with the main description.
...
#There's already a page with names and links all of the projects listed by alphabetical 
#order, which can use to make an index
...
#Through trial and error I know that the WIPO RE:Search project (#422) page isn't loading 
#correctly, so let's take that one out as well.
...
col_name <- c("Prog_dscr", "Company", "Partner", "Partner_type", "Therapeutic_focus", "Disease", 
              "Prog_type", "Target_pop", "Region", "Country", "Date_start", "Date_end")

col_selector <- c("//*[(@id = 'article-details')]", 
                  "//a[contains(@href, '/search?page=1&co[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&pa[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&ptf[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&t[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&d[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&pr[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&tp[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&re[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&c[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&y[]')]/text()", 
                  "//a[contains(@href, '/search?page=1&ey[]')]/text()")

#Create function to iterate over all the urls on our list, extracting the column info we 
#want from each page. Watch for NAs!
...
save(url, index_page, prog_url, prog_name, col_selector, col_name, col_extract, alltext, 
     file = "rda/IFPMA_db.rda")
